...
- **예약대기는 좌석이 아닙니다.** "좌석이 없으면 예약대기라도 신청"을 켜면 매진 열차에 **예약대기(취소표 순번)** 를
  등록합니다. 이것은 좌석을 확보한 것이 아니어서 코레일+ 앱의 일반 예약승차권 목록에는 보이지 않고 **예약대기 목록**에서
  확인됩니다. 결제기한도 없다가, 취소표가 나와 좌석이 배정되면 코레일+ 알림과 함께 결제기한이 안내되고 그때 결제해야
  확정됩니다. 기본 설정에서는 예약대기를 등록한 뒤에도 빈 좌석을 계속 찾으며, 빈 좌석을 예약하면 필요 없는 예약대기는
  코레일+ 앱에서 취소하세요.
...
- **"예약대기 등록"이라고 나오는데 코레일+ 앱의 예약 목록에 없음** — 예약대기는 일반 예약승차권 목록이 아니라
  코레일+ 앱의 **예약대기 목록**에서 확인합니다. 좌석이 배정되기 전까지는 결제기한도 없습니다.
